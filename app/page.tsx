...
export default function ShambaPlatform() {
  ...
  const [currentActiveRole, setCurrentActiveRole] = React.useState<
    "investor" | "farmer" | "worker" | "admin"
  >(routeRole);
  ...
  const [showInvestModal, setShowInvestModal] = React.useState<Farm | null>(
    null,
  );
  ...
  const [aiMatchedData, setAiMatchedData] = React.useState<AIMatchData | null>(
    null,
  );
  const [aiMatchingLoading, setAiMatchingLoading] = React.useState(false);

  // --- AI Anomaly Assessment Controls ---
  const [anomalyTargetFarm, setAnomalyTargetFarm] =
    React.useState<string>("farm-1");
  const [aiAnomalyData, setAiAnomalyData] =
    React.useState<AIAnomalyData | null>(null);
  ...
  const [propType, setPropType] = React.useState<
    "crops" | "livestock" | "aquaculture" | "cash_crops"
  >("crops");
  ...
  const [reportStatus, setReportStatus] = React.useState<"passed" | "flagged">(
    "passed",
  );
  ...
  const handleAIMatchRequest = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        logActivity(
          "Launched AI Matching",
          `Sought matching recommendations for budget of KES ${Number(aiBudget).toLocaleString()}`,
        );
      } else {
      ...
      alert("Network exception in matching: " + message);
    } finally {
  ...
  const handleAIAnomalyAssessment = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        logActivity(
          "Ran AI Anomaly Check",
          `Scanned logs of '${targetFarmObj.name}'`,
        );
      } else {
  ...
  const handleDeposit = (e: React.FormEvent) => {
    ...
    triggerNotification(
      "Capital Deposit Successful",
      `KES ${amt.toLocaleString()} credited successfully from your linked M-Pesa account.`,
      "success",
    );
    logActivity(
      "Wallet deposit",
      `Deposited KES ${amt.toLocaleString()} via integration API`,
    );
    setDepositAmount("");
  ...
  const handleWithdraw = (e: React.FormEvent) => {
    ...
    triggerNotification(
      ...
      `KES ${amt.toLocaleString()} successfully transferred to your linked mobile number.`,
      "success",
    );
    logActivity(
      "Wallet withdrawal",
      `Withdrew KES ${amt.toLocaleString()} via M-Pesa B2C integration`,
    );
    setWithdrawAmount("");
  ...
  const handleAcquireUnits = (e: React.FormEvent) => {
    ...
    if (investChamaId) {
      ...
      if (!targetChama || targetChama.totalBalance < cost) {
        alert(
          "Insufficient Chama syndicate pool balance to cover KES " +
            cost.toLocaleString(),
        );
        return;
      }
      ...
      alert(
        "Insufficient wallet balance to cover KES " + cost.toLocaleString(),
      );
      return;
    }
    ...
    const newInvestment: Investment = {
      ...
      expectedReturn: Math.round(
        cost * (1 + showInvestModal.expectedROI / 100),
      ),
      date: getTodayString(),
      status: "active",
  ...
  const handleCreateChama = (e: React.FormEvent) => {
    ...
    const newChama: Chama = {
      ...
      description:
        newChamaDesc || "Multi-user cooperative group agricultural pool.",
      members: [
        {
    ...
    if (walletBalance < 50000) {
      alert(
        "You need at least KES 50,000 in your wallet to initialize and seed a Chama group.",
      );
      return;
    }
    ...
    logActivity(
      "Spun up Chama",
      `Created Chama Group '${newChamaName}' and seeded it with KES 50,000`,
    );
    triggerNotification(
      "Chama Initialized",
      `Chama group '${newChamaName}' successfully registered. Swahili esusu rules applied.`,
      "success",
    );
  ...
  const handleMemberJoinChama = (chamaId: string) => {
    if (walletBalance < 25000) {
      alert(
        "Minimum buy-in for joining this Chama in this simulation is KES 25,000",
      );
      return;
    }
    ...
    logActivity(
      "Joined Chama group",
      `Contributed KES 25,000 to group reserves`,
    );
    triggerNotification(
      "Pooled capital contributed",
      "Joined Group syndicate successfully.",
      "success",
    );
  };
  ...
  const handleFarmerProposal = (e: React.FormEvent) => {
    ...
    const newFarm: Farm = {
      ...
      imageUrl:
        "https://picsum.photos/seed/harvest_" + getRandomSeed() + "/800/600",
      description:
        propDesc ||
        "Excellent agricultural enterprise with secured grain/product off-taker frameworks.",
    ...
    logActivity(
      "Registered Project Proposal",
      `Proposed '${propName}' for capital financing target: KES ${targetVal.toLocaleString()}`,
    );
    triggerNotification(
      "Enterprise Registered",
      `Successfully submitted proposal for '${propCrop}'. Held in administrative queue.`,
      "success",
    );
  ...
  const handleFarmerLogUpdate = (e: React.FormEvent) => {
    ...
    const newUpdate: FarmUpdate = {
      ...
      imageUrl:
        "https://picsum.photos/seed/update_" + getRandomSeed() + "/800/600",
    };
    ...
    logActivity(
      "Logged Farm Update",
      `Added progress category '${updateType}' update to Project ID ${updateFarmId}`,
    );
    triggerNotification(
      "Chronology Dispatched",
      `Development log successfully synchronized for Project of scale.`,
      "success",
    );
  ...
  const handleWorkerReport = (e: React.FormEvent) => {
    ...
    logActivity(
      ...
      `Audit completed for '${matchedFarm.name}' - Result: ${reportStatus.toUpperCase()}`,
    );
    triggerNotification(
      "Audit Locked in Ledger",
      `Site evaluation report uploaded. Status updated to ${reportStatus.toUpperCase()}`,
      isPassed ? "success" : "alert",
    );

    // Payout field agent reward simulation
    updateWalletState(walletBalance + 3500); // Field workers earn KES 3,500
    triggerNotification(
      "Incentive Transferred",
      "KES 3,500 physical audit service fee credited to Worker wallet.",
      "success",
    );
  ...
  const handleAdminApproveFarm = (farmId: string) => {
    ...
    logActivity(
      "Listing proposal approved",
      `Listing approved for regional physical safety inspections.`,
    );
    triggerNotification(
      "Listing Moved to Queue",
      "Project proposal approved relative to criteria; dispatched to field agents.",
      "success",
    );
  };
  ...
  const handleAdminTriggerPayout = (farmId: string) => {
    ...
    logActivity(
      "Authorized Harvest Settlement",
      `Released escrow to investors of ${targetFarm.name}. Harvest settlements total: KES ${totalPaidOut.toLocaleString()}`,
    );
    triggerNotification(
      "Escrow Released to Swahili Pools",
      `Succeeded harvest cycle payouts of KES ${totalPaidOut.toLocaleString()} backed by off-taker deals.`,
      "success",
    );
  };
  ...
  const handleAdminFlagSuspicious = (farmId: string) => {
    ...
    updateFarmsState(updated);
    logActivity(
      "Imposed Platform Safety Lock",
      `Imposed listing safety lockdown on Farm ID: ${farmId}`,
    );
    triggerNotification(
      "Platform Flag Issued",
      "Listing suspended. Capital releases temporarily frozen in escrow.",
      "alert",
    );
  };
  ...
  const totalInvestedCumulative = investments.reduce(
    (sum, current) => sum + current.amount,
    0,
  );
  const expectedReturnCumulative = investments.reduce(
    (sum, current) => sum + current.expectedReturn,
    0,
  );
  ...
  return (
    <div
      id="shamba-frame"
      className="min-h-screen bg-[#FDFCFB] text-[#1B3022] font-sans selection:bg-amber-100 antialiased"
    >
        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          <div className="flex flex-col">
            <div className="flex items-baseline gap-2">
              <span className="font-serif font-black text-3xl sm:text-4xl tracking-tighter uppercase leading-none text-[#1B3022]">
                FARM LINK
              </span>
              <span className="bg-[#D97757]/10 text-[#D97757] text-[9px] font-bold px-2 py-0.5 rounded-none border border-[#D97757]/20 uppercase tracking-widest font-mono">
                TRUST CORE
              </span>
            </div>
            <p className="text-[9px] tracking-[0.2em] font-semibold uppercase mt-1 text-[#D97757]">
              The Agricultural Trust Infrastructure
            </p>
          </div>
          ...
          <div className="p-0.5 flex flex-wrap items-center gap-1 border border-[#1B3022]/10 bg-[#F5F2EF] rounded-none">
            <span className="text-[9px] font-bold text-[#1B3022]/40 uppercase tracking-[0.2em] px-2.5 hidden xl:inline">
              ROLE:
            </span>
            {(
              [
              ...
              return (
                ...
                >
                  <Icon
                    className={`h-3.5 w-3.5 ${isActive ? "text-[#D97757]" : "text-[#1B3022]/40"}`}
                  />
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="bg-[#F5F2EF] border border-[#1B3022]/10 rounded-none p-2 px-3 flex items-center gap-3">
              ...
              <div className="text-right">
                <p className="text-[9px] text-[#1B3022]/50 font-bold uppercase tracking-[0.2em]">
                  Escrow Balance
                </p>
                <p className="font-mono text-xs font-black text-[#1B3022]">
                    ...
                    <div className="space-y-2 max-h-60 overflow-y-auto">
                      {notifications.length === 0 ? (
                        ...
                        notifications.map((not) => (
                          <div
                            key={not.id}
                            className={`p-2 rounded-xl text-xs border ${not.read ? "border-transparent bg-transparent" : "border-emerald-100 bg-emerald-50/50"}`}
                          >
      ...
      <div className="bg-[#F5F2EF] border-b border-[#1B3022]/10 py-3.5 px-4 text-center">
        <p className="text-[11px] text-[#1B3022]/80 uppercase tracking-widest font-semibold flex items-center justify-center gap-1.5">
          ...
          <span>
            Farm Link simulation model is active. Use the top sandbox switcher
            to experiment with Investor, Farmer, Field Worker and Compliance
            admin actions.
          </span>
        </p>
      </div>
          ...
          <div className="space-y-8">
            ...
            <div className="bg-[#1B3022] text-[#FDFCFB] rounded-none p-8 md:p-10 relative overflow-hidden border border-[#1B3022]/10">
              <div className="relative z-10 max-w-2xl">
                ...
                <p className="text-xs md:text-sm text-[#FDFCFB]/80 font-sans tracking-wide leading-relaxed mt-4">
                  Farm Linkutilizes legal escrow protections, licensed custodian
                  frameworks, and physical field audit telemetry to connect
                  urban capital securely to Kenyan agriculture.
                </p>
                <div className="mt-6 flex flex-wrap gap-3">
                  <a
                    href="#directories"
                    className="bg-[#D97757] hover:bg-[#c8623f] text-[#FDFCFB] px-5 py-2.5 rounded-none font-bold text-xs uppercase tracking-widest shadow transition"
                  >
                    Explore Verified Ventures
                  </a>
                  <a
                    href="#aimatcher"
                    className="bg-transparent hover:bg-white/5 text-[#FDFCFB] border border-white/20 px-5 py-2.5 rounded-none font-bold text-xs uppercase tracking-widest transition"
                  >
                    AI Profiling Engine
                  </a>
                </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
              <div className="bg-white p-6 rounded-none border border-[#1B3022]/10 shadow-sm transition hover:shadow-md">
                ...
                <p className="text-3xl font-serif text-[#1B3022] mt-2 font-medium">
                  {investments.filter((i) => i.status === "active").length}
                </p>
                <p className="text-[11px] text-[#1B3022]/60 mt-1 italic">
                  Direct vetted contracts
                </p>
              </div>
              <div className="bg-white p-6 rounded-none border border-[#1B3022]/10 shadow-sm transition hover:shadow-md">
                <p className="text-[10px] font-bold text-[#D97757] uppercase tracking-[0.2em]">
                ...
                <p className="text-3xl font-serif text-[#1B3022] mt-2 font-medium">
                  KES {expectedReturnCumulative.toLocaleString()}
                </p>
                <p className="text-[11px] text-[#1B3022]/60 mt-1 italic">
                  Based on fixed off-taking contracts
                </p>
              </div>
              <div className="bg-white p-6 rounded-none border border-[#1B3022]/10 shadow-sm transition hover:shadow-md">
                <p className="text-[10px] font-bold text-[#D97757] uppercase tracking-[0.2em]">
                ...
                <p className="text-3xl font-serif text-[#1B3022] mt-2 font-medium">
                  KES {totalInvestedCumulative.toLocaleString()}
                </p>
                <p className="text-[11px] text-[#1B3022]/60 mt-1 italic">
                  Escrow held locked safety
                </p>
              </div>
              <div className="bg-white p-6 rounded-none border border-[#1B3022]/10 shadow-sm transition hover:shadow-md">
                <p className="text-[10px] font-bold text-[#D97757] uppercase tracking-[0.2em]">
                ...
                <div className="flex items-center gap-2 mt-2">
                  <p className="text-3xl font-serif text-[#1B3022] font-medium">
                    {investorScore}/100
                  </p>
                  <span className="bg-[#D97757]/10 text-[#D97757] font-bold text-[8px] px-1.5 py-0.5 rounded-none border border-[#D97757]/20 tracking-wider">
                    EXCELLENT
                  </span>
                </div>
                <p className="text-[11px] text-[#1B3022]/60 mt-1 italic">
                  Unlocks lower checkout platform fees
                </p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <div className="lg:col-span-2 space-y-8">
                ...
                <div
                  id="aimatcher"
                  className="bg-[#F5F2EF] border border-[#1B3022]/10 rounded-none p-6 shadow-sm"
                >
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <h2 className="text-lg font-serif font-black text-[#1B3022] italic">
                        AI Farm Matchmaking Engine
                      </h2>
                      <p className="text-[11px] text-[#1B3022]/70">
                        Input constraints to build dynamic, multi-farm crop
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-5">
                    <div>
                      <label className="block text-[10px] font-bold text-[#D97757] uppercase tracking-wider mb-1.5">
                        Target Capital (KES)
                      </label>
                      <input
                        type="number"
                  ...
                  >
                    {aiMatchingLoading ? (
                      <>
                        ...
                        <span>
                          Querying Deep Match matrices via Gemini 3.5...
                        </span>
                      </>
                    ) : (
                      <>
                    ...
                    {aiMatchedData && (
                      ...
                      >
                        ...
                        <div className="space-y-3 mt-4">
                          <h3 className="text-[10px] font-bold text-[#D97757] tracking-[0.2em] uppercase">
                            Proportional Recommendations
                          </h3>
                          {aiMatchedData.recommendations &&
                            aiMatchedData.recommendations.map((rec, idx) => {
                              ...
                              return (
                                <div
                                  key={idx}
                                  className="bg-[#FDFCFB] border border-[#1B3022]/10 rounded-none p-3.5 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3"
                                >
                                  <div>
                                    <div className="flex items-center gap-2">
                                    ...
                                    <p className="text-[11px] text-[#1B3022]/60 mt-1">
                                      {rec.reason}
                                    </p>
                                  </div>
                                  <div className="text-right flex sm:flex-col items-center sm:items-end justify-between w-full sm:w-auto mt-2 sm:mt-0 pt-2 sm:pt-0 border-t sm:border-0 border-[#1B3022]/10">
                                    <span className="text-[9px] text-[#1B3022]/40 font-bold uppercase tracking-wider block">
                                      ALLOCATION SUGGESTION
                                    </span>
                                    <span className="font-mono text-xs font-bold text-[#1B3022]">
                                      KES{" "}
                                      {rec.allocationAmount?.toLocaleString()}
                                    </span>
                                    {matchingFarm && (
                                      <button
                                        onClick={() => {
                        ...
                        <div className="mt-4 pt-3 border-t border-[#1B3022]/10 bg-[#F5F2EF]/50 p-2.5 rounded-none border border-[#1B3022]/10 flex items-start gap-2 text-[11px] text-[#1B3022]">
                          ...
                          <p>
                            <strong>AI Escrow Safeguard Guard:</strong>{" "}
                            {aiMatchedData.riskReview}
                          </p>
                        </div>
                      </motion.div>
                    )}
                ...
                <div id="directories" className="space-y-6">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-[#1B3022]/10 pb-4">
                    <div>
                      <h2 className="text-2xl font-serif font-black text-[#1B3022]">
                        Verified Agricultural Ventures
                      </h2>
                      <p className="text-xs text-[#1B3022]/60">
                        Escrow backed fractional assets. Click to buy shares
                        (units).
                      </p>
                    </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    {filteredFarms.length === 0 ? (
                      <div className="col-span-full bg-[#F5F2EF] py-12 text-center rounded-none border border-[#1B3022]/10">
                        ...
                        <p className="text-xs font-bold uppercase tracking-wider text-[#1B3022]/50">
                      ...
                      filteredFarms.map((farm) => {
                        const percentage = Math.min(
                          100,
                          Math.round(
                            (farm.fundedCapital / farm.targetCapital) * 100,
                          ),
                        );
                          ...
                          >
                            <div className="relative h-48 w-full bg-[#F5F2EF]">
                              ...
                              <div className="absolute top-3 left-3 flex gap-1.5 flex-wrap">
                                ...
                                <span
                                  className={`text-[8px] font-mono font-bold px-2.5 py-1 rounded-none uppercase tracking-widest ${
                                    farm.riskLevel === "low"
                                      ? "bg-[#FDFCFB] border border-[#1B3022]/10 text-[#1B3022]"
                            ...
                            <div className="p-6 flex-1 flex flex-col justify-between">
                              <div>
                                <span className="text-[10px] font-bold text-[#D97757] uppercase tracking-[0.2em] mb-1 block">
                                  {farm.cropType}
                                </span>
                                <h3 className="font-serif font-black text-[#1B3022] text-xl leading-snug line-clamp-1">
                                  {farm.name}
                                </h3>
                              ...
                              <div className="mt-6 pt-4 border-t border-[#1B3022]/10 space-y-2">
                                <div className="flex justify-between items-center text-xs font-semibold tracking-wide uppercase text-[#1B3022]">
                                  ...
                                  <span className="font-mono text-[#1B3022]/60 text-[11px]">
                                    {farm.fundedUnits} / {farm.totalUnits} Units
                                  </span>
                                </div>
                                <div className="w-full bg-[#1B3022]/5 h-1.5 rounded-none overflow-hidden">
                                  <div
                                    className="h-full bg-[#1B3022] rounded-none"
                                ...
                                <div className="flex justify-between items-center text-[10px] text-[#1B3022]/50 uppercase tracking-wider font-semibold font-sans mt-2">
                                  <span>
                                    Ticket: KES{" "}
                                    {farm.unitPrice.toLocaleString()} / unit
                                  </span>
                                  <span>
                                    Goal: KES{" "}
                                    {farm.targetCapital.toLocaleString()}
                                  </span>
                                </div>
                              </div>
                              ...
                              <div className="mt-6 flex items-center justify-between gap-2.5 pt-4 border-t border-[#1B3022]/10">
                                <div className="text-left">
                                  <p className="text-[9px] text-[#1B3022]/40 font-bold uppercase tracking-[0.2em]">
                                    Farmer Score
                                  </p>
                                  <span className="font-mono text-xs font-black text-[#1B3022]">
                                    {farm.farmerCreditScore}/100 Badge
                                  </span>
                                </div>
                                {farm.status === "funding" &&
                                farm.verificationStatus === "verified" ? (
              ...
              <div className="space-y-8">
                ...
                <div className="bg-white border border-[#1B3022]/10 rounded-none p-6 shadow-sm">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <h2 className="text-md font-serif font-black text-[#1B3022]">
                        Chama Pools (Syndicates)
                      </h2>
                      <p className="text-[10px] text-[#1B3022]/60">
                        Form esusu circles to pool agricultural investments
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="space-y-3 mt-4">
                    {chamas.map((chama) => {
                      const isMember = chama.members.some(
                        (m) => m.userId === "investor-joel",
                      );
                      return (
                        <div
                          key={chama.id}
                          className="bg-[#F5F2EF]/50 border border-[#1B3022]/10 rounded-none p-3.5"
                        >
                          <div className="flex justify-between items-start">
                            <h3 className="text-xs font-serif font-black text-[#1B3022]">
                              {chama.name}
                            </h3>
                          ...
                          <p className="text-[10px] text-[#1B3022]/70 mt-1 leading-relaxed">
                            {chama.description}
                          </p>
                          <div className="flex justify-between items-center mt-3 pt-2 text-[11px] border-t border-[#1B3022]/10">
                            <div>
                              <p className="text-[#1B3022]/40 font-bold uppercase text-[8px] tracking-wider">
                                Group Capital Reserve
                              </p>
                              <span className="font-mono text-[#1B3022] font-black">
                                KES {chama.totalBalance.toLocaleString()}
                              </span>
                            </div>
                            {isMember ? (
                              <span className="bg-white border border-[#1B3022]/10 text-[#1B3022]/70 text-[9px] uppercase tracking-widest font-extrabold px-2.5 py-1 rounded-none font-mono">
                                Joined member
                              </span>
                            ) : (
                              <button
                  ...
                  {/* Creation form */}
                  <form
                    onSubmit={handleCreateChama}
                    className="mt-4 pt-4 border-t border-[#1B3022]/10 space-y-2.5"
                  >
                    <p className="text-[9px] font-bold text-[#D97757] uppercase tracking-[0.2em] mb-1">
                      Establish New Syndicate
                    </p>
                    <input
                      type="text"
                ...
                <div className="bg-white border border-[#1B3022]/10 rounded-none p-6 shadow-sm">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <div>
                      <h2 className="text-md font-serif font-black text-[#1B3022]">
                        Your Escrow Holdings
                      </h2>
                      <p className="text-[10px] text-[#1B3022]/60">
                        Live custody and payout schedule indicators
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="space-y-3">
                    {investments.map((inv) => {
                      return (
                        <div
                          key={inv.id}
                          className="border border-[#1B3022]/10 bg-[#FDFCFB] rounded-none p-4 hover:border-[#D97757] transition"
                        >
                          <div className="flex justify-between items-start">
                            <div>
                              <h3 className="text-xs font-serif font-bold text-[#1B3022] line-clamp-1">
                                {inv.farmName}
                              </h3>
                              <p className="text-[9px] text-[#1B3022]/50 uppercase font-mono mt-0.5">
                                Asset units: {inv.units} • KES{" "}
                                {inv.amount.toLocaleString()}
                              </p>
                            </div>
                            <span
                              className={`text-[8px] tracking-wider font-mono font-bold px-2 py-0.5 rounded-none uppercase ${
                          ...
                          <div className="flex justify-between items-center mt-3 pt-2 border-t border-[#1B3022]/10 text-[11px]">
                            <div>
                              <span className="text-[8px] text-[#1B3022]/40 font-bold block uppercase">
                                SECURED CUSTODY RELEASE
                              </span>
                              <strong className="text-[#1B3022] font-mono text-xs">
                                KES {inv.expectedReturn.toLocaleString()}
                              </strong>
                            </div>
                            ...
                            {inv.status === "active" && (
                              <button
                                onClick={() => {
                                  ...
                                  logActivity(
                                    "Filed Dispute on placement",
                                    `Raised a transparent dispute window against ${inv.farmName} custody`,
                                  );
                                  triggerNotification(
                                    "Dispute Flagged",
                                    "Capital release held locked. Compliance officer dispatching.",
                                    "alert",
                                  );
                                }}
                ...
                <div className="bg-[#1B3022] text-[#FDFCFB] rounded-none p-6 border-l-4 border-[#D97757]">
                  ...
                  <h3 className="font-serif italic text-base mt-2 leading-snug font-bold">
                    The Reason Predecessors Failed
                  </h3>
                  <p className="text-[11px] text-[#FDFCFB]/80 mt-2 leading-relaxed font-sans">
                    Former crowdsourcers acted as piggy-banks, scaling without
                    physical audits or regulatory compliance. SHAMBA holds money
                    strictly in independent licensed Escrow custodians. Funds
                    are only authorized for farmer payout once Field Workers
                    confirm land prep and output batches.
                  </p>
                </div>
              </div>
            </div>
        ...
        {currentActiveRole === "farmer" && (
          <div className="space-y-8">
            <div className="bg-[#1B3022] text-[#FDFCFB] p-6 rounded-none flex justify-between items-center border border-[#1B3022]/10">
              <div>
                <span className="bg-[#D97757]/15 border border-[#D97757]/30 text-[#D97757] text-[9px] font-bold px-2.5 py-1 rounded-none uppercase tracking-widest font-mono">
                  Farmer Terminal
                </span>
                <h1 className="text-2xl font-serif font-black mt-2">
                  Nshamba Agricultural Dashboard
                </h1>
                <p className="text-xs text-[#FDFCFB]/80 mt-1">
                  Ezekiel Kiprotich (Verified Rift Valley Agri-Manager)
                </p>
              </div>
              <div className="bg-white/10 border border-white/20 p-2 text-center backdrop-blur-xs min-w-28 font-mono">
                <p className="text-[8px] text-[#D97757] font-extrabold uppercase tracking-wider">
                  Verified Cred Index
                </p>
                <p className="text-3xl font-black">94/100</p>
                <div className="text-[8px] mt-0.5 text-[#D97757] font-bold uppercase tracking-wider">
                  ● Premium Tier
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <div className="lg:col-span-1 bg-white border border-[#1B3022]/10 rounded-none p-6 shadow-sm">
                ...
                <p className="text-[11px] text-[#1B3022]/60 mb-4">
                  Request seasonal crop/fencing credit lines. Placed under
                  escrow review rules.
                </p>

                <form
                  onSubmit={handleFarmerProposal}
                  className="space-y-3.5 text-xs"
                >
                  <div>
                    <label className="block text-[9px] font-bold text-[#D97757] tracking-wider uppercase mb-1">
                      VENTURE PROFILE NAME
                    </label>
                    <input
                      type="text"
              ...
              <div className="lg:col-span-1 bg-white border border-[#1B3022]/10 rounded-none p-6 shadow-sm">
                ...
                <p className="text-[11px] text-[#1B3022]/60 mb-4 font-normal">
                  Broadcast physical updates. Real-time geo-pinned coordinates
                  will be logged automatically.
                </p>

                <form
                  onSubmit={handleFarmerLogUpdate}
              ...
              <div className="lg:col-span-1 space-y-4">
                <h2 className="text-base font-serif font-black text-[#1B3022]">
                  My Platform Listings
                </h2>

                <div className="space-y-4">
                  {farms
                    .filter((f) => f.farmerId === "farmer-ezekiel")
                    .map((farm) => {
                      const pct = Math.min(
                        100,
                        Math.round(
                      ...
                      return (
                        <div
                          key={farm.id}
                          className="bg-white border border-[#1B3022]/10 rounded-none p-4 shadow-sm space-y-4"
                        >
                          <div className="flex justify-between items-start">
                            <div>
                              <span className="text-[8px] bg-[#F5F2EF] text-[#1B3022]/60 px-1.5 py-1 border border-[#1B3022]/10 rounded-none font-bold font-mono uppercase tracking-widest">
                                {farm.cropType}
                              </span>
                              <h3 className="text-xs font-serif font-black text-[#1B3022] mt-2 line-clamp-1">
                                {farm.name}
                              </h3>
                            </div>
                            <span
                              className={`text-[8px] font-bold font-mono tracking-widest uppercase px-1.5 py-0.5 rounded-none ${
                                farm.verificationStatus === "verified"
                                  ? "bg-[#1B3022]/10 text-[#1B3022]"
                                  : "bg-[#F5F2EF] text-[#1B3022]/60"
                              }`}
                            >
                          ...
                          <div className="space-y-1.5">
                            ...
                            <div className="text-[10px] text-[#1B3022]/60 font-mono mt-1 font-bold">
                              KES {farm.fundedCapital.toLocaleString()} / KES{" "}
                              {farm.targetCapital.toLocaleString()}
                            </div>
                          </div>
                          ...
                          <div className="pt-3 border-t border-[#1B3022]/10 text-[11px] space-y-2">
                            <p className="text-[9px] text-[#D97757] font-bold uppercase tracking-wider">
                              Dispatched log timeline ({farm.updates.length})
                            </p>
                            {farm.updates.slice(0, 2).map((up, ui) => (
                              <div
                                key={ui}
                                className="bg-[#F5F2EF]/55 p-2.5 border border-[#1B3022]/5 rounded-none"
                              >
                                <div className="flex justify-between items-center text-[10px] text-[#1B3022]/50 font-mono">
                                  <span className="font-bold text-[#D97757] uppercase text-[8px]">
                                    {up.type}
                                  </span>
                                  <span>
                                    {new Date(
                                ...
                                <p className="font-serif font-bold text-[#1B3022] text-xs mt-1">
                                  {up.title}
                                </p>
                                <p className="text-[10px] text-[#1B3022]/70 leading-relaxed mt-0.5">
                                  {up.description}
                                </p>
                              </div>
                            ))}
          ...
          <div className="space-y-8">
            <div className="bg-[#1B3022] text-[#FDFCFB] p-6 rounded-none flex justify-between items-center border border-[#1B3022]/10 shadow-sm">
              <div>
                ...
                <h1 className="text-2xl font-serif font-black mt-2">
                  Mobile Agronomic Auditing
                </h1>
                <p className="text-xs text-[#FDFCFB]/80 mt-1">
                  Joseph Kuria (Landed Agent #EA-7741 - Nakuru)
                </p>
              </div>
              <div className="bg-white/10 border border-white/20 p-2 text-center backdrop-blur-xs min-w-36 font-mono">
                <p className="text-[8px] text-[#D97757] font-extrabold uppercase tracking-wider">
                  Audit Services Earned
                </p>
                <p className="text-2xl font-black text-[#D97757]">
                  KES {(reports.length * 3500).toLocaleString()}
                </p>
                <div className="text-[8px] mt-0.5 text-stone-300">
                  @ KES 3,500 / audit report
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <div className="lg:col-span-1 bg-white border border-[#1B3022]/10 rounded-none p-6 shadow-sm">
                ...
                <p className="text-[11px] text-[#1B3022]/60 mb-4">
                  Complete physical check list, inspect soil maps, and log
                  coordinate stamps to unlock investor financing.
                </p>
                ...
                >
                    ...
                    <div className="flex gap-2">
                      <button
                        ...
                        className={`flex-1 font-bold py-2 px-1 rounded-none transition border text-center text-[10px] uppercase tracking-wider ${
                          reportStatus === "passed"
                            ? "bg-[#1B3022]/10 border-[#1B3022] text-[#1B3022]"
                            : "bg-stone-50 border-[#1B3022]/10 text-stone-600"
                        }`}
                      >
                        Passed (Clear)
                      </button>
                      <button
                        ...
                        className={`flex-1 font-bold py-2 px-1 rounded-none transition border text-center text-[10px] uppercase tracking-wider ${
                          reportStatus === "flagged"
                            ? "bg-red-50 border-red-500 text-red-850 font-extrabold"
                            : "bg-stone-50 border-[#1B3022]/10 text-stone-600"
                        }`}
                      >
                        Flag Anomaly
                  ...
                  <div>
                    ...
                    >
                      <option value="5">
                        5 Stars (Excellent / fully verified records)
                      </option>
                      <option value="4">
                        4 Stars (Compliant but minor repairs needed)
                      </option>
                      <option value="3">
                        3 Stars (Messy buffers / missing recent bills)
                      </option>
                      <option value="2">
                        2 Stars (Critical updates missing / uncooperative)
                      </option>
                    </select>
                  </div>
                  ...
                  <div>
                    <label className="block text-[9px] font-bold text-[#D97757] tracking-wider uppercase mb-1">
                      PROFESSIONAL INSPECTION AUDIT NOTES
                    </label>
                    <textarea
                      rows={4}
                      className="w-full bg-white border border-[#1B3022]/20 text-[#1B3022] rounded-none p-2 outline-none focus:border-[#D97757] transition resize-none"
              ...
              <div className="lg:col-span-2 space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {farms.map((f) => (
                    <div
                      key={f.id}
                      className="bg-white border border-[#1B3022]/10 rounded-none p-4 shadow-sm space-y-3"
                    >
                      <div className="flex justify-between items-start">
                        <div>
                          ...
                          <h3 className="text-xs font-serif font-black text-[#1B3022] line-clamp-1 mt-1.5">
                            {f.name}
                          </h3>
                          <p className="text-[10px] text-[#1B3022]/50 mt-0.5">
                            Manager: {f.farmerName}
                          </p>
                        </div>
                        <span
                          className={`text-[8px] font-bold font-mono tracking-widest uppercase px-1.5 py-0.5 rounded-none ${
                      ...
                      <div className="text-[11px] text-[#1B3022]/70 font-sans">
                        ...
                        <span>
                          Coords: {f.locationCoords.lat}, {f.locationCoords.lng}{" "}
                          • <strong>{f.sizeAcres} Acres</strong>
                        </span>
                      </div>
                      ...
                      <div className="pt-2 border-t border-[#1B3022]/10 flex justify-between items-center text-[10px]">
                        <span className="font-sans uppercase text-[#1B3022]/50 font-bold text-[9px] tracking-wider">
                          Commission:{" "}
                          <strong className="text-[#1B3022] font-mono">
                            KES 3,500
                          </strong>
                        </span>
                        <button
                          onClick={() => {
                            ...
                            logActivity(
                              "Began physical audit process",
                              `Driving routes mapped for ${f.name}`,
                            );
                          }}
          ...
          <div className="space-y-8 col-span-3">
            <div className="bg-[#1B3022] text-[#FDFCFB] p-6 rounded-none flex justify-between items-center border border-[#1B3022]/10 mb-2">
              <div>
                ...
                <h1 className="text-2xl font-serif font-black mt-2">
                  Platform Safeguard Control Desk
                </h1>
                <p className="text-xs text-[#FDFCFB]/80 mt-1">
                  Escrow allocations tracking & Fraud Telemetry Audits
                </p>
              </div>
              <div className="bg-white/10 border border-white/20 p-2 text-center backdrop-blur-xs min-w-36 font-mono">
                <p className="text-[8px] text-[#D97757] font-extrabold uppercase tracking-wider">
                  System-wide Default Index
                </p>
                <p className="text-2xl font-black text-green-400">0.0%</p>
                <div className="text-[8px] mt-0.5 text-[#FDFCFB]/60">
                  100% payout completion
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <div className="lg:col-span-2 space-y-8">
                ...
                <div className="bg-[#F5F2EF] border border-[#1B3022]/10 rounded-none p-6 shadow-sm">
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <div>
                      <h2 className="text-base font-serif font-black text-[#1B3022]">
                        AI Telemetry Anomaly Auditor
                      </h2>
                      <p className="text-[10px] text-[#1B3022]/60">
                        Scans timeline inputs against regional cultivation
                        growth cycle structures via Gemini AI
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col sm:flex-row items-center gap-3">
                    <div className="flex-1 w-full">
                      <label className="block text-[9px] font-bold text-[#D97757] uppercase mb-1 tracking-wider">
                        Select an active harvest land listing
                      </label>
                      <select
                    ...
                    {aiAnomalyData && (
                      ...
                      >
                        <div className="flex items-center justify-between border-b border-[#1B3022]/10 pb-2">
                          ...
                          <span
                            className={`text-[9px] font-bold font-mono tracking-widest uppercase px-2 py-0.5 rounded-none ${
                              aiAnomalyData.riskScore > 50
                                ? "bg-red-50 text-red-800 border border-red-200"
                                : "bg-[#1B3022]/10 text-[#1B3022]"
                            }`}
                          >
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                          <div>
                            <p className="text-[9px] text-[#D97757] font-bold uppercase tracking-wider mb-1.5 font-mono">
                              Flagged Findings
                            </p>
                            <ul className="space-y-1.5">
                              {aiAnomalyData.findings &&
                                aiAnomalyData.findings.map(
                                  (finding: string, idx: number) => (
                                    <li
                                      key={idx}
                                      className="text-xs text-[#1B3022]/85 flex items-start gap-1 pb-1 border-b border-[#1B3022]/5 leading-relaxed font-sans"
                                    >
                              ...
                              {(!aiAnomalyData.findings ||
                                aiAnomalyData.findings.length === 0) && (
                                <p className="text-xs text-[#1B3022]/50 italic">
                                  No anomalous metrics raised by criteria.
                                </p>
                              )}
                            </ul>
                          </div>
                          ...
                          <div className="bg-[#F5F2EF]/50 rounded-none p-3.5 border border-[#1B3022]/10">
                            <p className="text-[9px] text-[#1B3022]/50 font-bold uppercase tracking-wider font-mono">
                              Agronomic Risk Profile
                            </p>
                            ...
                            <div className="flex justify-between items-center mt-2.5">
                              <span className="text-[9px] text-[#D97757] font-bold tracking-wider uppercase font-mono">
                        ...
                        <div className="flex gap-2.5 mt-4 pt-3 border-t border-[#1B3022]/10">
                          {aiAnomalyData.recommendedAction === "suspend" && (
                            <button
                              onClick={() =>
                                handleAdminFlagSuspicious(anomalyTargetFarm)
                              }
                          ...
                          <button
                            onClick={() => {
                              ...
                              logActivity(
                                "Cleared Farm Security Review",
                                "Manually override risk evaluation logs as fully compliant",
                              );
                              triggerNotification(
                                "Safety Clear",
                ...
                <div className="space-y-4">
                  <h2 className="text-base font-serif font-black text-[#1B3022]">
                    Compliance Auditing Queues
                  </h2>

                  <div className="space-y-3">
                    {farms.length === 0 ? (
                      <p className="text-xs text-[#1B3022]/40 text-center py-6 font-serif">
                        Listing pipeline is clear.
                      </p>
                    ) : (
                      farms.map((farm) => (
                        <div
                          key={farm.id}
                          className="bg-white border border-[#1B3022]/10 rounded-none p-4 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-4"
                        >
                          <div>
                            <div className="flex items-center gap-2">
                              <span className="text-[8px] bg-[#F5F2EF] text-[#1B3022]/60 px-1.5 py-0.5 rounded-none border border-[#1B3022]/10 font-bold font-mono tracking-widest uppercase">
                            ...
                            <p className="text-[10px] text-[#1B3022]/70 mt-1">
                              Owner:{" "}
                              <strong>
                                {farm.farmerName} (Viability:{" "}
                                {farm.farmerCreditScore}/100)
                              </strong>
                            </p>
                            <p className="text-[10px] text-[#1B3022]/70">
                              Capital requested:{" "}
                              <strong>
                                KES {farm.targetCapital.toLocaleString()}
                              </strong>{" "}
                              • Status:{" "}
                              <strong className="text-[#D97757] uppercase font-bold tracking-wider text-[9px]">
                                {farm.status}
                              </strong>
                            </p>
                          </div>
                          ...
                          <div className="flex flex-wrap items-center gap-2.5">
                            ...
                            {farm.status === "active" && (
                              <button
                                onClick={() =>
                                  handleAdminTriggerPayout(farm.id)
                                }
                ...
                <div className="space-y-4 mt-8">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {reports.length === 0 ? (
                      <p className="text-xs text-[#1B3022]/40 col-span-full py-4 italic">
                        ...
                        >
                          <div className="flex justify-between items-start">
                            <div>
                              ...
                              <p className="text-[10px] text-[#1B3022]/50">
                                Auditor: {report.workerName}
                              </p>
                            </div>
                            <span
                              className={`text-[8px] font-bold font-mono tracking-widest uppercase px-1.5 py-0.5 rounded-none ${
                          ...
                          <div className="flex items-center gap-1">
                            {[...Array(5)].map((_, i) => (
                              <Star
                                key={i}
                                className={`h-2.5 w-2.5 ${i < report.rating ? "text-amber-400 fill-amber-400" : "text-stone-200"}`}
                              />
                            ))}
                          </div>
                          ...
                          <div className="pt-2 border-t border-[#1B3022]/10 flex justify-between items-center">
                            <span className="text-[9px] text-[#1B3022]/40 font-mono">
              ...
              <div className="space-y-6">
                <div className="bg-[#FDFCFB] border border-[#1B3022]/10 rounded-none p-6 shadow-sm">
                  <h2 className="text-base font-serif font-black text-[#1B3022] mb-3">
                    Escrow ledger activity logs
                  </h2>
                  ...
                  <div className="space-y-3.5 max-h-[450px] overflow-y-auto pr-1">
                    {logs.map((log) => (
                      <div
                        key={log.id}
                        className="text-xs pb-3 border-b border-[#1B3022]/5 font-normal"
                      >
                        <div className="flex justify-between text-[10px] text-[#1B3022]/40 font-medium">
                          <span className="font-extrabold uppercase text-[#D97757] font-mono tracking-wider text-[8px]">
                            {log.userRole}
                          </span>
                          <span className="font-mono text-[9px] text-[#1B3022]/40">
                            {new Date(log.timestamp).toLocaleTimeString()}
                          </span>
                        </div>
                        <p className="font-serif font-black text-[#1B3022] mt-0.5">
                          {log.action}
                        </p>
                        <p className="text-[11px] text-[#1B3022]/60 mt-0.5 font-sans leading-relaxed">
                          {log.details}
                        </p>
                      </div>
                    ))}
      ...
      {showDepositModal && (
        <div className="fixed inset-0 bg-[#121212]/75 backdrop-blur-xs flex items-center justify-center z-50 p-4">
          ...
          >
            <h3 className="font-serif font-black text-lg text-[#1B3022]">
              Credit Escrow Capital
            </h3>
            <p className="text-[11px] text-[#1B3022]/60 mt-1 font-sans">
              Simulates mobile payment checkout via local KES API interfaces.
            </p>

            <form onSubmit={handleDeposit} className="space-y-4 mt-4">
              <div>
                <label className="block text-[9px] font-bold text-[#D97757] uppercase mb-1 tracking-wider font-mono">
                  CAPITAL CONTRIBUTION VALUE (KES)
                </label>
                <div className="relative">
                  <span className="absolute left-3 top-2.5 font-bold text-stone-400 text-xs font-mono">
                    KES
                  </span>
                  <input
                    type="number"
              ...
              <div className="bg-[#F5F2EF] text-[#1B3022] p-2.5 rounded-none border border-[#1B3022]/10 text-[10px] leading-relaxed font-sans">
                <strong>Protected Placement:</strong> Contribution funds are
                held securely in a licensed custodian account, governed strictly
                under physical verification rules.
              </div>
      ...
      {showWithdrawModal && (
        <div className="fixed inset-0 bg-[#121212]/75 backdrop-blur-xs flex items-center justify-center z-50 p-4">
          ...
          >
            <h3 className="font-serif font-black text-lg text-[#1B3022]">
              Withdraw Escrow Capital
            </h3>
            <p className="text-[11px] text-[#1B3022]/60 mt-1 font-sans">
              Transfer available liquidity to your verified mobile money
              account.
            </p>
            ...
            <form onSubmit={handleWithdraw} className="space-y-4 mt-4">
              <div>
                ...
                <p className="text-[9px] text-[#1B3022]/40 mt-1.5 font-mono">
                  Available: KES {walletBalance.toLocaleString()}
                </p>
              </div>

              <div className="bg-[#F5F2EF] text-[#1B3022] p-2.5 rounded-none border border-[#1B3022]/10 text-[10px] leading-relaxed font-sans">
                <strong>Liquidity Notice:</strong> Withdrawals are processed
                instantly via our B2C disbursement API. Standard carrier
      ...
      {showInvestModal && (
        <div className="fixed inset-0 bg-[#121212]/75 backdrop-blur-xs flex items-center justify-center z-50 p-4">
          ...
          >
            <h3 className="font-serif font-black text-lg text-[#1B3022]">
              Acquire Shamba Units (Shares)
            </h3>
            <p className="text-[11px] text-[#1B3022]/60 mt-1 font-sans">
              Vetting: ID {showInvestModal.id} • Verified management metrics
              apply.
            </p>
            ...
            <form onSubmit={handleAcquireUnits} className="space-y-4 mt-4">
              <div className="grid grid-cols-2 gap-4 bg-[#F5F2EF] p-3 rounded-none border border-[#1B3022]/10 font-mono">
                <div>
                  <p className="text-[8px] text-[#1B3022]/50 font-bold uppercase tracking-wider">
                    UNIT INCREMENT COST
                  </p>
                  <span className="font-mono text-xs font-black text-[#1B3022]">
                    KES {showInvestModal.unitPrice.toLocaleString()}
                  </span>
                </div>
                <div>
                  <p className="text-[8px] text-[#1B3022]/50 font-bold uppercase tracking-wider">
                    EXPECTED ROI CONTRACT
                  </p>
                  <span className="text-xs font-black text-[#D97757]">
                    +{showInvestModal.expectedROI}% expected
                  </span>
                </div>
              </div>
              ...
              <div>
                <label className="block text-[9px] font-bold text-[#D97757] uppercase mb-1 tracking-wider font-mono">
                  ACQUISITION SIZE (UNITS)
                </label>
                <input
                  type="number"
                  min="1"
                  max={String(
                    showInvestModal.totalUnits - showInvestModal.fundedUnits,
                  )}
              ...
              <div>
                <label className="block text-[9px] font-bold text-[#D97757] uppercase mb-1 tracking-wider font-mono">
                  INVEST VIA SYNDICATE POOL (OPTIONAL)
                </label>
                <select
                ...
                >
                  <option value="">
                    Direct individual allocation (Escrow wallet)
                  </option>
                  {chamas.map((c) => (
                    <option key={c.id} value={c.id}>
                      {c.name} (Pool reserve: KES{" "}
                      {c.totalBalance.toLocaleString()})
                    </option>
                  ))}
                </select>
              </div>
              ...
              <div className="flex justify-between items-center text-xs pt-2 border-t border-[#1B3022]/10 font-mono">
                <span className="text-[#1B3022]/50 font-bold uppercase text-[9px]">
                  TOTAL ESCROW LOCK VALUE:
                </span>
                <strong className="font-mono text-sm font-black text-[#1B3022]">
                  KES{" "}
                  {(
                    Number(investUnits) * showInvestModal.unitPrice
                  ).toLocaleString()}
                </strong>
              </div>
      ...
      <footer className="bg-[#1B3022] text-[#FDFCFB]/50 border-t-2 border-[#1B3022] py-12 px-4 text-center mt-12 text-xs">
        <div className="max-w-7xl mx-auto space-y-3">
          <div className="flex items-center justify-center gap-2">
            ...
            <span className="font-serif italic font-bold text-[#FDFCFB]">
              Farm Link Escrow Systems Africa
            </span>
          </div>
          <p className="max-w-md mx-auto text-[11px] leading-relaxed text-[#FDFCFB]/70 font-sans">
            Registered securities intermediary trial playground. Licensed
            physical custody escrow model designed in Nairobi, Kenya. Built in
            full-stack App Router compliance, integrating modern Gemini match
            matrices.
          </p>
          <p className="text-[10px] text-[#FDFCFB]/40 font-mono uppercase tracking-widest mt-2">
            © 2026 Farm Link. All rights reserved.
          </p>
        </div>
      </footer>
    </div>
